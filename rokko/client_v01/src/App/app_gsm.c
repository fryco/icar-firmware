...
const unsigned char dest_server[] = DEST_SERVER ;
const unsigned char callback_phone[] = CALL_BACK_NUMBER;
...
static unsigned char send_tcp_data( unsigned char *, unsigned short * );
...
//2,Uart2 无反馈时，超时，重置状态 online ...
	while ( 1 ) {
		...
		if ( my_icar.mg323.power_on ) {//below action need power on
			...
			if ( my_icar.mg323.tcp_online ) { //can send data
				...
				if ( c2s_data.tx_sn_len && my_icar.need_sn ) {
					...
					if ( ((c2s_data.tx_len)*2 > GSM_BUF_LENGTH) \
						|| c2s_data.tx_timer == 0 \
		...
		if ( OSTime - relay_timer > my_icar.mg323.ring_count*my_icar.para.relay_on*OS_TICKS_PER_SEC ) { //10 mins.
			;//led_off(RELAY_LED);//shutdown relay
			my_icar.mg323.ring_count = 0 ;
			my_icar.mg323.need_dial = true ;
		}
		else {// < my_icar.mg323.ring_count*10 mins, open relay
			;//led_on(RELAY_LED); //for Relay
		}
...
//0: ok
//others: err
static unsigned char send_tcp_data(unsigned char *buffer, unsigned short *buf_len )
{
	if ( gsm_ask_tcp(*buf_len) ) {//GSM buffer ready
		if ( gsm_send_tcp(buffer,*buf_len) ) {
			//Check: ^SISW: 0,1 
			debug_gsm("Send %d bytes OK!\r\n",*buf_len);
			...
			*buf_len = 0 ;
			...
			prompt("send err, len:%d Check %s:%d\r\n",*buf_len,__FILE__, __LINE__);
			return 1 ;
		}
	}
	else {//may GSM module no respond, need to enquire

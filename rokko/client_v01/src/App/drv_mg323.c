...
bool set_dest_ip( )
{
	...
	for ( retry = 0 ;retry < 3 ; retry++) {
		...
		//putstring(COM1,"AT^SISS=0,address,\"socktcp:\/\/");
		putstring(COM2,my_icar.mg323.server_ip_port);
	 	putstring(COM1,my_icar.mg323.server_ip_port);
		putstring(COM2,"\"\r\n");
		memset(respond_str, 0x0, AT_CMD_LENGTH);
		if ( my_icar.stm32_u2_rx.empty ) {//no data...
			OSTimeDlyHMSM(0, 0,	0, 100);
...
unsigned char gsm_power_on( )
{
	...
	OSTimeDlyHMSM(0, 0, 0, 100);//wait power stable
	GSM_SW_ON; OSTimeDlyHMSM(0, 0, 1, 0); GSM_SW_OFF;
